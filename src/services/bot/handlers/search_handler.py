...
async def search(message: types.Message, state: FSMContext):
    ...
    print(cross_number, message.text)
    ...
    if info.get('error'):
        ...
        await message.answer(f'По данному серийному номеру({data.get('serial_number')}) было найдено\nДень в который необходимо оплатить - {info.get('pay_date')}, ваша почта - {info.get('email')}')
        await state.clear()